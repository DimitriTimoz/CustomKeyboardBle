...
void BleKeyboard::printReport(KeyReport* report){
  Serial.print("Report: ");
  Serial.print(report->modifiers, BIN);
  Serial.print(" | ");
  for (int i = 0; i < 8; i++) {
...
size_t BleKeyboard::write(uint8_t c)
{
	...
	release(c);            // Keyup
	return p;              // just test: test: test: return the result of press() since release() almost always returns 1
}
